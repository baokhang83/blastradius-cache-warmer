# Session: parse and filter Tier B dependency coordinates

- **intent:** parse and filter Tier B dependency coordinates
- **started:** 2026-08-07
...
- **DependencyTreeParser** — converts Maven dependency-tree coordinate lines into typed coordinates, retaining an optional classifier and the resolved scope · status: documented
- **DependencyManifestBuilder** — filters parsed coordinates to transferable third-party JARs and orders them by Maven repository path for stable Tier B publishing · status: documented
...
- **Test scope remains part of the manifest** — CI verification requires its test libraries, while provided and system dependencies remain the build environment's responsibility · status: documented
...
## Decision: build a deterministic third-party JAR manifest

- **where:** `dependency/DependencyManifestBuilder`
- **why:** T10 and T11 need exact local-repository paths, while reactor outputs and provided or system dependencies must not be cached as third-party slices.
- **alternative:** Keep raw dependency-tree output — rejected: later stages cannot address transferable files deterministically.

# Session: T6 fail open when Tier A key computation cannot read sources

- **intent:** T6 fail open when Tier A key computation cannot read sources
- **started:** 2026-08-07
...
- **Key-computation boundary** — a Tier A key comes from walking the source tree, so an unreadable
  source path is handled as an explicit skipped warm before any cache request or output mutation
  occurs · status: documented
...
- **Optimization errors stay non-fatal** — key computation joins cache misses, cache transport
  errors, and bad archives on the cold-build path, preserving the safety contract at every stage
  before Maven compilation · status: documented
...
## Decision: fail open when Tier A key computation fails

- **where:** `src/main/java/io/github/baokhang83/blastradius/cachewarmer/warmer/SiblingBytecodeWarmer.java`
- **why:** The source-tree hash is part of an optional optimization, so an unreadable source tree must leave Maven to run a cold correct build rather than propagate an exception.
- **alternative:** Propagate the key-computation exception - rejected: an optimization-only filesystem failure would break the build it is meant to accelerate.
